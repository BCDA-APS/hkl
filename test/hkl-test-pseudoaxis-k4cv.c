...
#define SET_AXES(geometry, komega, kappa, kphi, tth) do{	\
		double values[] = {komega * HKL_DEGTORAD,	\
				   kappa * HKL_DEGTORAD,	\
				   kphi * HKL_DEGTORAD,		\
				   tth * HKL_DEGTORAD};		\
	hkl_geometry_set_values_v(geometry, values, 4);		\
	} while(0)
...
{
	HklPseudoAxisEngineHklK4CV *engine = hkl_pseudo_axis_engine_hkl_k4_cv_new();
	hkl_pseudo_axis_engine_unref(engine);
...
HKL_TEST_SUITE_FUNC(degenerated)
{
	...
	geom = hkl_geometry_factory_new(HKL_GEOMETRY_TYPE_KAPPA4C_VERTICAL, parameters, 1);
...
HKL_TEST_SUITE_FUNC(q)
{
	...
	HklPseudoAxisEngine *engine;
	HklGeometry *geom;
	HklDetector *detector = hkl_detector_new(1);
	HklSample *sample;
	size_t i, f_idx;
	double *Q;
	double parameters[] = {50 * HKL_DEGTORAD};
